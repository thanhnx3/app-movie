...
import { Play, Star, Tv, ChevronLeft, ChevronRight } from "lucide-react";
...
function TV() {
  ...
  // ID TV giả lập cho demo - trong ứng dụng thực tế sẽ lấy từ useParams
  const tvId = "phim-bo";

  const fetchTVShows = async (page = 1) => {
    setLoading(true);
    try {
      const response = await fetch(`https://phimapi.com/v1/api/danh-sach/${tvId}?limit=24&page=${page}`);
      const tvData = await response.json();
      
      console.log("Dữ liệu TV:", tvData);
      
      if (tvData.data) {
        setTV(tvData.data.items);
        setTitle(tvData.data.titlePage);
        setTotalPages(tvData.data.params?.pagination?.totalPages || 1);
        document.title = tvData.data.seoOnPage.titleHead;
      }
    } catch (error) {
          ...
          return (
            ...
            >
              ...
              <div className="w-full aspect-[2/3] bg-gray-900 rounded-lg overflow-hidden shadow-lg transition-all duration-300 group-hover:shadow-xl group-hover:scale-105 relative">
                ...
                {/* Icon loại phim bộ */}
                <div className="absolute bottom-2 left-2 bg-purple-600/90 text-xs font-bold text-white px-2 py-1 rounded flex items-center gap-1">
                  <Tv size={12} />
                  PHIM BỘ
                </div>
              </div>
